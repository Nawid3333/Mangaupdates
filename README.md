...
- **Option 4 — Anime-Planet export**: scrapes your public [Anime-Planet](https://www.anime-planet.com/) profile and exports every non-empty manga and anime list to `exports/anime-planet/<timestamp>/` with one JSON file per list and a `_manifest.json`, just like option 1. Kept in its own subtree, separate from option 1's `exports/<timestamp>/`, so a MangaUpdates run is never diffed against an Anime-Planet snapshot or counted toward its rotation limit, or vice versa. Empty lists are skipped automatically. Because Anime-Planet has no public API, this reads the public HTML profile and list pages; it does not log in to Anime-Planet and does not need an Anime-Planet password, only the username in `.env`
...
- **Fails loudly where guessing would be worse** — a malformed _list page_ or list index aborts the export instead of being worked around, naming the list and page. A silently shortened list would be saved as if it were complete, and the next run would report every missing series as removed
...
3. **Configure credentials**
   ...
   Edit `.env` and fill in your MangaUpdates username and password, and your
   Anime-Planet username if you want to use option 4:

   ```
   MU_USERNAME=your_username
   MU_PASSWORD=your_password
   AP_USERNAME=your_anime_planet_username
   ```

   `AP_USERNAME` is public — it only builds the profile URL
   `https://www.anime-planet.com/users/{AP_USERNAME}`. No Anime-Planet password
   is needed.
...
You'll see your logged-in account and whether the API is reachable, then a menu to pick option 1, 2, 3, or 4 (repeatable — it returns to the menu after each one, until you choose `0`).

Option 1's exports are saved to `exports/<timestamp>/` and option 4's to `exports/anime-planet/<timestamp>/` — separate trees, each with its own `_manifest.json` and its own rotation, so the two sites' snapshots never diff against or count toward each other. Options 2 and 3 each write a single stable, always-overwritten report: `exports/related.txt` and `exports/ready_to_read.txt`.
...
| `LIST_PAGE_WORKERS`     | `8`     | Concurrent threads for list page requests (options 1 and 4). Page 1 reports `total_hits`, so the whole page range is fetched in one go rather than walked                                                                 |
