...
Option 1's and option 4's exports are saved to `exports/<timestamp>/` with one JSON file per list, plus a `_manifest.json` recording which file each list was saved to. Options 2 and 3 each write a single stable, always-overwritten report: `exports/related.txt` and `exports/ready_to_read.txt`.
